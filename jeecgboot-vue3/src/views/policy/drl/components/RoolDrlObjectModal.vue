  ...
  <BasicModal v-bind="$attrs" @register="registerModal" destroyOnClose :title="title" :width="800" @ok="handleSubmit">
    ...
    <a-tabs v-model:activeKey="activeKey" animated @change="handleChangeTabs">
      <a-tab-pane tab="规则文件DRL" key="roolDataDrlCode" :forceRender="true">
        <RoolDataDrlCodeForm ref="roolDataDrlCodeForm" :disabled="formDisabled" />
      </a-tab-pane>
    </a-tabs>
  </BasicModal>
</template>
...
<script lang="ts" setup>
  ...
  import { formSchema } from '../RoolDrlObject.data';
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    if (unref(isUpdate)) {
      ...
      roolDataDrlCodeForm.value.initFormData(roolDataDrlCodeList, data?.record?.id);
    }
    // 隐藏底部时禁用整个表单
  ...
  async function reset() {
    await resetFields();
    activeKey.value = 'roolDataDrlCode';
    roolDataDrlCodeForm.value.resetFields();
  }
  ...
  function validateSubForm(allValues) {
    return new Promise((resolve, reject) => {
      Promise.all([roolDataDrlCodeForm.value.validateForm(0)])
        .then(() => {
          resolve(allValues);
        })
        .catch((e) => {
